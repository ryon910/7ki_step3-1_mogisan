...
SPREADSHEET_ID = '1eLkYSugOUGFNrvA28loGBh4TUQpB1J934-zl4aGk4s0'
...
def display_search_results(filtered_df):
    ...
    display_columns = ['物件番号', '名称', 'アドレス', '階数', '家賃', '間取り', '面積','最寄り駅','物件詳細URL']
...
def main():
    ...
    with col1:
        # エリア選択
        selected_areas = st.multiselect('■ エリア選択', df['区'].unique())

    # 最寄り駅をエリア選択に基づいてフィルタリング
    if selected_areas:
        area_filtered_df = df[df['区'].isin(selected_areas)]
        available_stations = area_filtered_df['最寄り駅'].str.split(',').explode().unique()
    else:
        available_stations = df['最寄り駅'].str.split(',').explode().unique()
    ...
    with col2:
        ...
        # 最寄り駅選択
        nearest_station = st.multiselect(
            '■ 最寄り駅選択',
            available_stations
        )
    ...
    filtered_df = df[df['区'].isin(selected_areas)]
    filtered_df = filtered_df[filtered_df['間取り'].isin(type_options)]
    filtered_df = filtered_df[(filtered_df['家賃'] >= price_min) & (filtered_df['家賃'] <= price_max)]
    if nearest_station:
        filtered_df = filtered_df[filtered_df['最寄り駅'].str.split(',').apply(lambda x: any(station in x for station in nearest_station))]
