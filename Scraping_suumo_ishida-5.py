...
# データフレームに緯度と経度を追加する関数
def add_lat_lon(dataframe):
    geolocator = Nominatim(user_agent="real_estate_scraper")
    latitude = []
    longitude = []

    for address in dataframe['アドレス']:
        try:
...
def main():
    ...
    # デバッグ: 緯度と経度が正しく追加されているか確認
    print("緯度と経度が追加されたデータ:")
    print(df[['アドレス', 'latitude', 'longitude']].head())
